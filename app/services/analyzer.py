...
class RepositoryAnalyzer:
    ...
    def analyze(self) -> dict:
        ...
        for path in files:
            ...
            if dependency_kind == "lockfile":
                ...
                dependencies.update(
                    self._detect_dependencies(dependency_kind, dependency_content)
                )
                if dependency_kind == "package":
                    entry_points.update(
                        self._detect_package_entry_points(
                            path, dependency_content, files
                        )
                    )
    ...
    def _detect_package_entry_points(
        self, package_path, content: str, files: list
    ) -> set[str]:
        ...
        values.extend(self._package_export_values(document.get("exports")))

        scanned_files = {path.resolve() for path in files}
        entry_points = set()
        for value in values:
            entry_point = self._resolve_entry_point(
                package_path, value, scanned_files
            )
            if entry_point:
    ...
    def _package_export_values(self, value) -> list[str]:
        ...
        if isinstance(value, dict):
            ...
            for nested_value in value.values():
                values.extend(self._package_export_values(nested_value))
            return values
    ...
    def _resolve_entry_point(self, package_path, value, scanned_files) -> str | None:
        ...
        candidate = (package_path.parent / normalized_value).resolve()

        if candidate not in scanned_files:
            return None

        try:
            relative_path = candidate.relative_to(self.repository.root_path.resolve())
        except ValueError:
            return None

        return relative_path.as_posix()

    def _dependency_file_kind(self, filename: str) -> str | None:
    ...
    def _detect_dependencies(self, kind: str, content: str) -> set[str]:
    ...
    def _detect_requirements_dependencies(self, content: str) -> set[str]:
        dependencies = set()
        ...
        for line in content.splitlines():
            ...
            if egg_match:
                dependencies.add(egg_match.group(1).lower())
                continue
    ...
    def _detect_pyproject_dependencies(self, content: str) -> set[str]:
        try:
        ...
        if isinstance(project, dict):
            ...
            if isinstance(project_dependencies, list):
                dependency_values.extend(project_dependencies)
            optional_dependencies = project.get("optional-dependencies", {})
            if isinstance(optional_dependencies, dict):
                for values in optional_dependencies.values():
                    dependency_values.extend(values if isinstance(values, list) else [])
        ...
        if isinstance(poetry, dict):
            ...
            if isinstance(poetry_dependencies, dict):
                dependency_values.extend(
                    name for name in poetry_dependencies if name.lower() != "python"
                )
            poetry_dev_dependencies = poetry.get("dev-dependencies", {})
            if isinstance(poetry_dev_dependencies, dict):
                dependency_values.extend(poetry_dev_dependencies)

        return {
            dependency
            for value in dependency_values
            if isinstance(value, str)
            for dependency in [self._dependency_name(value)]
            if dependency
        }

    def _detect_package_dependencies(self, content: str) -> set[str]:
        try:
            document = json.loads(content)
        except (json.JSONDecodeError, TypeError):
            return set()
        ...
        for section in sections:
            ...
            if isinstance(values, dict):
                dependencies.update(
                    name.lower() for name in values if isinstance(name, str)
                )

        return dependencies

    def _dependency_name(self, value: str) -> str | None:
        match = re.match(r"\s*([A-Za-z0-9][A-Za-z0-9._-]*)", value)
        return match.group(1).lower() if match else None

    def _detect_frameworks(self, content: str, suffix: str) -> set[str]:
        if suffix == ".py":
    ...
    def _detect_python_frameworks(self, content: str) -> set[str]:
        ...
        for node in ast.walk(tree):
            ...
            for module_name in module_names:
                framework = framework_names.get(module_name.split(".", 1)[0])
                if framework:
                    frameworks.add(framework)

        return frameworks

    def _detect_javascript_frameworks(self, content: str) -> set[str]:
        tokens = self._tokenize_javascript(content)
        frameworks = set()

        for index, (token_type, token) in enumerate(tokens):
            if token_type == "identifier" and token == "import":
                module_name = self._imported_module_name(tokens, index)
            elif token_type == "identifier" and token == "require":
                module_name = self._required_module_name(tokens, index)
            else:
                continue
            ...
            if module_name == "react":
                ...
                module_name == "next" or module_name.startswith("next/")
            ):
                frameworks.add("Next.js")

        return frameworks

    def _tokenize_javascript(self, content: str) -> list[tuple[str, str]]:
        tokens = []
        index = 0
            ...
            if character.isalpha() or character in {"_", "$"}:
                ...
                while index < len(content):
                    ...
                    if not (character.isalnum() or character in {"_", "$"}):
                        break
                    index += 1
                tokens.append(("identifier", content[start:index]))
                continue
    ...
    def _imported_module_name(
        ...
        for offset, (token_type, token) in enumerate(
            ...
            if offset == index + 1 and token not in {"from"}:
                continue
    ...
    def _required_module_name(
        ...
        if index + 3 < len(tokens) and tokens[index + 1][1] == "(":
            module_type, module_name = tokens[index + 2]
            if module_type == "string" and tokens[index + 3][1] == ")":
                return module_name
        return None
